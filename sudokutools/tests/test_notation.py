...
BNPN_2x4 = (
    ...
    ("b1p3", "b1p4", "b2p3", "b2p4", "b3p3", "b3p4", "b4p3", "b4p4"),
    ("b1p5", "b1p6", "b2p5", "b2p6", "b3p5", "b3p6", "b4p5", "b4p6"),
    ("b1p7", "b1p8", "b2p7", "b2p8", "b3p7", "b3p8", "b4p7", "b4p8"),
    ("b5p1", "b5p2", "b6p1", "b6p2", "b7p1", "b7p2", "b8p1", "b8p2"),
    ("b5p3", "b5p4", "b6p3", "b6p4", "b7p3", "b7p4", "b8p3", "b8p4"),
    ("b5p5", "b5p6", "b6p5", "b6p6", "b7p5", "b7p6", "b8p5", "b8p6"),
    ("b5p7", "b5p8", "b6p7", "b6p8", "b7p7", "b7p8", "b8p7", "b8p8"),
)
...
class EncodeTests(TestCase):
    ...
    def test_encode_rncn_examples(self):
        """rncn encoding works for 3x3 sudokus."""
        for example, width, height in RNCN_EXAMPLES:
            for row, columns in enumerate(example):
                for col, coords in enumerate(columns):
                    self.assertEqual(coords, encode(((row, col), ), width=width,
                        height=height, use_boxes=False))
...
class DecodeTests(TestCase):
    def test_split_groups(self):
        """Decoding groups works."""
        self.assertEqual(((0, 0), (0, 1), (8, 8)), decode("r1c12,r9c9"))
        self.assertEqual(((0, 0), (0, 1), (9, 9)), decode(
            "r1c1,2,r10c10", width=2, height=5))

    def test_decode_rncn(self):
        """rncn decoding works for the given examples."""
        for example, width, height in RNCN_EXAMPLES:
            for row, columns in enumerate(example):
                for col, coords in enumerate(columns):
                    self.assertEqual(((row, col), ),
                                     decode(coords, width=width, height=height))

    def test_decode_bnpn(self):
        """bnpn decoding works for the given examples."""
        for example, width, height in BNPN_EXAMPLES:
            for row, columns in enumerate(example):
                for col, coords in enumerate(columns):
                    self.assertEqual(((row, col), ),
                                     decode(coords, width=width, height=height))
    ...
    def test_all_cells(self):
        """Decoding returns all cells for strings like 'b1'."""
        for char, func in TEST_FUNCTIONS:
